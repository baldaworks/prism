...
3. **Prism agents** discoverable by Callee so `callee agent list` shows `prism/*`. If absent, install them with `callee agent import`.

### Install Prism agents when absent

If `callee agent list | grep '^prism/'` returns nothing, install the Prism catalog into the current project root:
...
2. Confirm pack: `callee agent list | grep '^prism/'`. If missing, run `callee agent import baldaworks/prism --path pack/callee/prism --prefix prism`, then re-check.
...
2. When importing Prism agents, use `--prefix prism` so imported IDs stay under `prism/*`.
