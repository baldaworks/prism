# Prism Light Lifecycle Architecture

This document defines only the simplified Prism Light host lifecycle:
`$prism:light`, `/prism:light`, and `/prism-light`. It does not define Callee
automation or allow `callee agent run prism/...`.

## Purpose

Prism Light preserves the pre-existing concise host workflow. It intentionally does
not claim parity with the full Callee role contracts.

The Prism Light skill performs each concise phase directly and persists results in Beads:
...
| Concern | Files |
| --- | --- |
| Manual host lifecycle skill | `plugins/prism/skills/light/` |
| Flat-name mirror | `plugins/prism/prefixed-skills/prism-light/` |
| Shared lifecycle graph | `plugins/prism/skills/light/references/lifecycle.md` |
| Phase instructions | `plugins/prism/skills/light/references/*.md` and prefixed mirror |
